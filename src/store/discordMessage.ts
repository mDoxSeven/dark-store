...
export const DISCORD_BANNER_URL = 'https://raw.githubusercontent.com/mDoxSeven/dark-store/main/public/contas-discord-banner-dark.png';
export const DISCORD_SELECT_ID = 'store:discord:select';
...
export function discordCatalogMessage(products: CatalogProduct[]) {
  ...
  return {
    ...
    components: [{ type: 17, accent_color: 0xaeb1b6, components: [
      { type: 12, items: [{ media: { url: DISCORD_BANNER_URL }, description: 'Catálogo Discord da dark store' }] },
      ...
      { type: 1, components: [{ type: 3, custom_id: DISCORD_SELECT_ID, placeholder: available.length ? 'Selecione uma opção' : 'Sem itens disponíveis', min_values: 1, max_values: 1, disabled: !available.length, options }] },
